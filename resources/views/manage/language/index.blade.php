...
<div class="col-12 m-b-10">
    <a href="{{ route('language.create') }}" class="btn btn-success">@lang('common.create')</a>
</div>
<div class="col-12">
    {!! Form::open(['route' => ['language.destroy', 0], 'method' => 'DELETE']) !!}
        <button type="submit"
            class="btn btn-danger m-b-10" 
            title="@lang('common.deletes_message', ['name' => __('language.language')])"
            onclick="{{ "return confirm('".__('common.deletes_message',
                            ['name' => __('language.language')])."')" }}">
        ...
        <div class="table-responsive">
            <table id="multipeCheckbox" class="table table-striped table-earning table-bordered">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="width-42">{{ Form::checkbox('ids[]', $language->id, null, ['class' => 'checkbox-delete']) }}</td>
                        <td>{{ $language->name }}</td>
                        <td class="text-center">
                            <a href="{{ route('language.edit', $language->id) }}" 
                            class="fas fa-edit" 
                            title="{{ __('common.edit_title', 
                                        ['name' => $language->name]) }}"></a>
                        </td>
                        <td class="text-center">
                            {{ Form::button(null,
                                [
                                    ...
                                    'title' => __('common.delete_message', ['name' => __('language.language')]),
                                    'onclick' => "return confirm('".__('common.delete_message',
                                    [
                                        'table_name' => __('language.language'),
                                        'name' => $language->name
                                    ])."')"
                                ]) }}
                        </td>
